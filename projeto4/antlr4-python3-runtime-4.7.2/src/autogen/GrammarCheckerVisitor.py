...
class GrammarCheckerVisitor(ParseTreeVisitor):
    ...
    def visitStatement(self, ctx:GrammarParser.StatementContext):
        if ctx.RETURN() != None:
            ...
            if ir_register != None:
                out = "    ret " + llvm_type(tyype) + " %" + str(ir_register) + "\n"
                self.filee.write(out)
            else:
                out = "    ret " + llvm_type(tyype) + " " + str(cte_value) + "\n"
                self.filee.write(out)
                
            function_type, params, cte_value, function_ir_register = self.ids_defined[self.inside_what_function]
    ...
    def visitVariable_assignment(self, ctx:GrammarParser.Variable_assignmentContext):
        ...
        if op == '++' or op == '--':
            ...
            if ir_register != None:
                ...
                if op == '++':
                    out = "add nsw " + llvm_type(tyype) + " %" + str(ir_register) + ", 1\n"
                    self.filee.write(out)
                elif op == '--':
                    out = "sub nsw " + llvm_type(tyype) + " %" + str(ir_register) + ", 1\n"
                    self.filee.write(out)
            ...
            if ir_register != None:
                ...
                if op == '+=':
                    if expr_ir_register != None:
                        out = "add nsw " + llvm_type(tyype) + " %" + str(ir_register) + ", %" + str(expr_ir_register) + "\n"
                        self.filee.write(out)
                    else:
                        out = "add nsw " + llvm_type(tyype) + " %" + str(ir_register) + ", " + str(expr_cte_value) + "\n"
                        self.filee.write(out)
                elif op == '-=':
                    if expr_ir_register != None:
                        out = "sub nsw " + llvm_type(tyype) + " %" + str(ir_register) + ", %" + str(expr_ir_register) + "\n"
                        self.filee.write(out)
                    else:
                        out = "sub nsw " + llvm_type(tyype) + " %" + str(ir_register) + ", " + str(expr_cte_value) + "\n"
                        self.filee.write(out)
                elif op == '*=':
                    if expr_ir_register != None:
                        out = "mul nsw " + llvm_type(tyype) + " %" + str(ir_register) + ", %" + str(expr_ir_register) + "\n"
                        self.filee.write(out)
                    else:
                        out = "mul nsw " + llvm_type(tyype) + " %" + str(ir_register) + ", " + str(expr_cte_value) + "\n"
                        self.filee.write(out)
                elif op == '/=':
                    if expr_ir_register != None:
                        out = "sdiv nsw " + llvm_type(tyype) + " %" + str(ir_register) + ", %" + str(expr_ir_register) + "\n"
                        self.filee.write(out)
                    else:
                        out = "sdiv nsw " + llvm_type(tyype) + " %" + str(ir_register) + ", " + str(expr_cte_value) + "\n"
                        self.filee.write(out) 
        ...
        if len(ctx.expression()) == 0:
            ...
            if ctx.OP != None: #unary operators
                ...
                if ir_register != None and text == '-':
                    out = "    %" + str(self.next_ir_register) + " = "
                    self.filee.write(out)
                    out = "sub nsw " + llvm_type(tyype) + " 0, %" + str(ir_register) + "\n"
                    self.filee.write(out)   
                    ir_register = self.next_ir_register
                ...
                if left_ir_register != None or right_ir_register != None:
                    ...
                    if text == '*':
                        if left_ir_register != None and right_ir_register != None:
                            out = "mul nsw " + llvm_type(tyype) + " %" + str(left_ir_register) + ", %" + str(right_ir_register) + "\n"
                            self.filee.write(out)
                        elif left_ir_register != None and right_ir_register == None:
                            out = "mul nsw " + llvm_type(tyype) + " %" + str(left_ir_register) + ", " + str(right_cte_value) + "\n"
                            self.filee.write(out)
                        elif left_ir_register == None and right_ir_register != None:
                            out = "mul nsw " + llvm_type(tyype) + " " + str(left_cte_value) + ", %" + str(right_ir_register) + "\n"
                            self.filee.write(out)

                    elif text == '/':
                        if left_ir_register != None and right_ir_register != None:
                            out = "sdiv nsw " + llvm_type(tyype) + " %" + str(left_ir_register) + ", %" + str(right_ir_register) + "\n"
                            self.filee.write(out)
                        elif left_ir_register != None and right_ir_register == None:
                            out = "sdiv nsw " + llvm_type(tyype) + " %" + str(left_ir_register) + ", " + str(right_cte_value) + "\n"
                            self.filee.write(out)
                        elif left_ir_register == None and right_ir_register != None:
                            out = "sdiv nsw " + llvm_type(tyype) + " " + str(left_cte_value) + ", %" + str(right_ir_register) + "\n"
                            self.filee.write(out)
                            
                    elif text == '+':
                        if left_ir_register != None and right_ir_register != None:
                            out = "add nsw " + llvm_type(tyype) + " %" + str(left_ir_register) + ", %" + str(right_ir_register) + "\n"
                            self.filee.write(out)
                        elif left_ir_register != None and right_ir_register == None:
                            out = "add nsw " + llvm_type(tyype) + " %" + str(left_ir_register) + ", " + str(right_cte_value) + "\n"
                            self.filee.write(out)
                        elif left_ir_register == None and right_ir_register != None:
                            out = "add nsw " + llvm_type(tyype) + " " + str(left_cte_value) + ", %" + str(right_ir_register) + "\n"
                            self.filee.write(out)

                    elif text == '-':
                        if left_ir_register != None and right_ir_register != None:
                            out = "sub nsw " + llvm_type(tyype) + " %" + str(left_ir_register) + ", %" + str(right_ir_register) + "\n"
                            self.filee.write(out)
                        elif left_ir_register != None and right_ir_register == None:
                            out = "sub nsw " + llvm_type(tyype) + " %" + str(left_ir_register) + ", " + str(right_cte_value) + "\n"
                            self.filee.write(out)
                        elif left_ir_register == None and right_ir_register != None:
                            out = "sub nsw " + llvm_type(tyype) + " " + str(left_cte_value) + ", %" + str(right_ir_register) + "\n"
                            self.filee.write(out)
    ...
    def visitFunction_call(self, ctx:GrammarParser.Function_callContext):
        ...
        for i in range(len(ctx.expression())):
            ...
            arg_func[i] = arg_type, arg_cte_value, arg_ir_register
    
        if tyype != Type.VOID:
            out = "    %" + str(self.next_ir_register) + " = call " + llvm_type(tyype) + " @" + name + "(" 
            self.filee.write(out)
            ir_register = self.next_ir_register
            self.next_ir_register += 1
            for i in range(len(ctx.expression())):
                
                arg_type, arg_cte_value, arg_ir_register = arg_func[i]
                
                if arg_ir_register != None:
                    if i != (len(ctx.expression()) - 1):
                        out = llvm_type(arg_type) + " %" + str(arg_ir_register) + " ,"
                        self.filee.write(out)
                    else:
                        out = llvm_type(arg_type) + " %" + str(arg_ir_register) + ")\n"
                        self.filee.write(out)
                    ...
                    if i != (len(ctx.expression()) - 1):
                        out = llvm_type(arg_type) + " " + str(int(arg_cte_value)) + " ,"
                        self.filee.write(out)
                    else:
                        out = llvm_type(arg_type) + " " + str(int(arg_cte_value)) + ")\n"
                        self.filee.write(out)
